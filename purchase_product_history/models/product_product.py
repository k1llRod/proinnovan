...
class ProductProduct(models.Model):
    ...
    po_product_line_ids = fields.One2many(
        'purchase.product.history.line', 'product_history_id',
        string='Purchase History', help='Purchased product variant details',
        ...
        if int(limit) >= 0 and status:
            ...
            if not limit:
                product_po_order_line = order_line.filtered(
                    ...
                    line.product_id.id == self.id and line.state in state)
            else:
            ...
            self.env['purchase.product.history.line'].create([{
                ...
                'name_partner': rec.partner_id.name,
